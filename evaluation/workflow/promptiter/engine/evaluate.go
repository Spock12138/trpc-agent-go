...
// EvaluateWithProfile evaluates one eval set input against an explicit profile.
func (e *engine) EvaluateWithProfile(
	ctx context.Context,
	input EvalSetInput,
	profile *promptiter.Profile,
	...
	return e.evaluate(ctx, structure, &EvaluationRequest{
		EvalSets: []EvalSetInput{input},
		Profile:  profile,
	})
}
...
type CaseResult struct {
	...
	Trace *atrace.Trace
	// ActualInvocation stores the first run invocation generated by the agent when available.
	ActualInvocation *evalset.Invocation
	// ExpectedInvocation stores the first run expected invocation when available.
...
func adaptEvaluationCaseResult(
	...
	actualInvocation, expectedInvocation := firstRunInvocations(runResult)
	return &CaseResult{
		EvalSetID:          evalSetID,
...
func firstRunInvocations(runResult *evalresult.EvalCaseResult) (*evalset.Invocation, *evalset.Invocation) {
	if runResult == nil || len(runResult.EvalMetricResultPerInvocation) == 0 {
		return nil, nil
	}
	perInvocation := runResult.EvalMetricResultPerInvocation[0]
