...
func (m *fakeModel) Info() model.Info {
	return model.Info{Name: "phase1v2-fake-model"}
}
...
func (m *fakeModel) generate(request *model.Request) *model.Response {
	...
	if directNoToolResponse(userText) {
		return fakeTextResponse("ready")
	}
...
func directNoToolResponse(userText string) bool {
	lower := strings.ToLower(userText)
	return strings.Contains(lower, "without looking anything up") &&
		(strings.Contains(lower, "just say ready") || strings.Contains(lower, "just say: ready"))
}
...
func (o *fakeOptimizer) Optimize(_ context.Context, request *optimizer.Request) (*optimizer.Result, error) {
	...
	return &optimizer.Result{
		Patch: &promptiter.SurfacePatch{
			...
			Reason: "Phase 1 fake optimizer adds flight capability words to lookup_record.",
		},
	}, nil
}
